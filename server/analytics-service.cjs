const { db } = require('./db');
const { 
  revenueTransactions, 
  monthlyRevenue, 
  userSessions, 
  courseProgress, 
  userEngagement, 
  courseAnalytics, 
  courseInteractions, 
  userFeedback,
  dailyAnalytics,
  weeklyAnalytics,
  monthlyAnalytics
} = require('../shared/analytics-schema');
const { eq, and, gte, lte, desc, asc, sql, count, sum, avg } = require('drizzle-orm');

class AnalyticsService {
  constructor() {
    this.db = db;
  }

  // Revenue Analytics Methods
  async trackRevenueTransaction(transactionData) {
    try {
      const result = await this.db.insert(revenueTransactions).values({
        id: transactionData.id,
        userId: transactionData.userId,
        courseId: transactionData.courseId,
        amount: transactionData.amount.toString(),
        currency: transactionData.currency || 'USD',
        paymentMethod: transactionData.paymentMethod,
        stripeSessionId: transactionData.stripeSessionId,
        status: transactionData.status,
        completedAt: transactionData.completedAt,
        refundedAt: transactionData.refundedAt,
        metadata: transactionData.metadata
      }).returning();

      // Update monthly revenue
      await this.updateMonthlyRevenue(transactionData);
      
      return result[0];
    } catch (error) {
      console.error('Error tracking revenue transaction:', error);
      throw error;
    }
  }

  async updateMonthlyRevenue(transactionData) {
    try {
      const date = new Date(transactionData.completedAt || new Date());
      const year = date.getFullYear();
      const month = date.getMonth() + 1;

      // Check if monthly record exists
      const existing = await this.db.select()
        .from(monthlyRevenue)
        .where(and(
          eq(monthlyRevenue.year, year),
          eq(monthlyRevenue.month, month)
        ));

      if (existing.length > 0) {
        // Update existing record
        await this.db.update(monthlyRevenue)
          .set({
            totalRevenue: sql`${monthlyRevenue.totalRevenue} + ${transactionData.amount}`,
            totalTransactions: sql`${monthlyRevenue.totalTransactions} + 1`,
            updatedAt: new Date()
          })
          .where(and(
            eq(monthlyRevenue.year, year),
            eq(monthlyRevenue.month, month)
          ));
      } else {
        // Create new record
        await this.db.insert(monthlyRevenue).values({
          id: `${year}-${month}`,
          year,
          month,
          totalRevenue: transactionData.amount.toString(),
          totalTransactions: 1,
          newCustomers: 1, // This should be calculated properly
          averageOrderValue: transactionData.amount.toString()
        });
      }
    } catch (error) {
      console.error('Error updating monthly revenue:', error);
    }
  }

  async getRevenueAnalytics(startDate, endDate) {
    try {
      const transactions = await this.db.select()
        .from(revenueTransactions)
        .where(and(
          gte(revenueTransactions.createdAt, startDate),
          lte(revenueTransactions.createdAt, endDate),
          eq(revenueTransactions.status, 'completed')
        ))
        .orderBy(desc(revenueTransactions.createdAt));

      const monthlyData = await this.db.select()
        .from(monthlyRevenue)
        .where(and(
          gte(sql`CONCAT(${monthlyRevenue.year}, '-', LPAD(${monthlyRevenue.month}, 2, '0'))`, 
              startDate.toISOString().substring(0, 7)),
          lte(sql`CONCAT(${monthlyRevenue.year}, '-', LPAD(${monthlyRevenue.month}, 2, '0'))`, 
              endDate.toISOString().substring(0, 7))
        ))
        .orderBy(asc(monthlyRevenue.year), asc(monthlyRevenue.month));

      // Calculate metrics
      const totalRevenue = transactions.reduce((sum, t) => sum + parseFloat(t.amount), 0);
      const totalTransactions = transactions.length;
      const averageOrderValue = totalTransactions > 0 ? totalRevenue / totalTransactions : 0;
      const uniqueCustomers = new Set(transactions.map(t => t.userId)).size;

      return {
        totalRevenue,
        totalTransactions,
        averageOrderValue,
        uniqueCustomers,
        monthlyBreakdown: monthlyData,
        transactions: transactions.slice(0, 100) // Limit for performance
      };
    } catch (error) {
      console.error('Error getting revenue analytics:', error);
      throw error;
    }
  }

  // User Engagement Methods
  async trackUserSession(sessionData) {
    try {
      const result = await this.db.insert(userSessions).values({
        id: sessionData.id,
        userId: sessionData.userId,
        sessionStart: sessionData.sessionStart,
        sessionEnd: sessionData.sessionEnd,
        duration: sessionData.duration,
        pageViews: sessionData.pageViews,
        coursesAccessed: sessionData.coursesAccessed,
        deviceType: sessionData.deviceType,
        browser: sessionData.browser,
        location: sessionData.location,
        referrer: sessionData.referrer
      }).returning();

      return result[0];
    } catch (error) {
      console.error('Error tracking user session:', error);
      throw error;
    }
  }

  async trackCourseProgress(progressData) {
    try {
      const result = await this.db.insert(courseProgress).values({
        id: progressData.id,
        userId: progressData.userId,
        courseId: progressData.courseId,
        enrollmentDate: progressData.enrollmentDate,
        completionDate: progressData.completionDate,
        progressPercentage: progressData.progressPercentage,
        timeSpent: progressData.timeSpent,
        lastAccessed: progressData.lastAccessed,
        isCompleted: progressData.isCompleted,
        certificateIssued: progressData.certificateIssued
      }).returning();

      return result[0];
    } catch (error) {
      console.error('Error tracking course progress:', error);
      throw error;
    }
  }

  async getUserEngagementMetrics(startDate, endDate) {
    try {
      const sessions = await this.db.select()
        .from(userSessions)
        .where(and(
          gte(userSessions.sessionStart, startDate),
          lte(userSessions.sessionStart, endDate)
        ));

      const progress = await this.db.select()
        .from(courseProgress)
        .where(and(
          gte(courseProgress.enrollmentDate, startDate),
          lte(courseProgress.enrollmentDate, endDate)
        ));

      // Calculate metrics
      const totalSessions = sessions.length;
      const uniqueUsers = new Set(sessions.map(s => s.userId)).size;
      const averageSessionDuration = sessions.reduce((sum, s) => sum + (s.duration || 0), 0) / totalSessions;
      const totalEnrollments = progress.length;
      const totalCompletions = progress.filter(p => p.isCompleted).length;
      const completionRate = totalEnrollments > 0 ? (totalCompletions / totalEnrollments) * 100 : 0;

      // Calculate retention (users who returned within 7 days)
      const retentionData = await this.calculateUserRetention(startDate, endDate);

      return {
        totalSessions,
        uniqueUsers,
        averageSessionDuration,
        totalEnrollments,
        totalCompletions,
        completionRate,
        retentionRate: retentionData.retentionRate,
        dailyActiveUsers: await this.getDailyActiveUsers(startDate, endDate)
      };
    } catch (error) {
      console.error('Error getting user engagement metrics:', error);
      throw error;
    }
  }

  async calculateUserRetention(startDate, endDate) {
    try {
      // Get users who registered in the period
      const newUsers = await this.db.select()
        .from(userSessions)
        .where(and(
          gte(userSessions.sessionStart, startDate),
          lte(userSessions.sessionStart, endDate)
        ));

      const userIds = [...new Set(newUsers.map(s => s.userId))];
      let retainedUsers = 0;

      for (const userId of userIds) {
        // Check if user had another session within 7 days
        const firstSession = newUsers.find(s => s.userId === userId);
        const sevenDaysLater = new Date(firstSession.sessionStart.getTime() + 7 * 24 * 60 * 60 * 1000);
        
        const returnSession = await this.db.select()
          .from(userSessions)
          .where(and(
            eq(userSessions.userId, userId),
            gte(userSessions.sessionStart, sevenDaysLater)
          ))
          .limit(1);

        if (returnSession.length > 0) {
          retainedUsers++;
        }
      }

      const retentionRate = userIds.length > 0 ? (retainedUsers / userIds.length) * 100 : 0;

      return {
        totalNewUsers: userIds.length,
        retainedUsers,
        retentionRate
      };
    } catch (error) {
      console.error('Error calculating user retention:', error);
      return { totalNewUsers: 0, retainedUsers: 0, retentionRate: 0 };
    }
  }

  async getDailyActiveUsers(startDate, endDate) {
    try {
      const dailyData = [];
      const currentDate = new Date(startDate);
      
      while (currentDate <= endDate) {
        const dayStart = new Date(currentDate);
        const dayEnd = new Date(currentDate);
        dayEnd.setHours(23, 59, 59, 999);

        const daySessions = await this.db.select()
          .from(userSessions)
          .where(and(
            gte(userSessions.sessionStart, dayStart),
            lte(userSessions.sessionStart, dayEnd)
          ));

        const uniqueUsers = new Set(daySessions.map(s => s.userId)).size;
        
        dailyData.push({
          date: currentDate.toISOString().split('T')[0],
          activeUsers: uniqueUsers
        });

        currentDate.setDate(currentDate.getDate() + 1);
      }

      return dailyData;
    } catch (error) {
      console.error('Error getting daily active users:', error);
      return [];
    }
  }

  // Content Performance Methods
  async trackCourseInteraction(interactionData) {
    try {
      const result = await this.db.insert(courseInteractions).values({
        id: interactionData.id,
        userId: interactionData.userId,
        courseId: interactionData.courseId,
        interactionType: interactionData.interactionType,
        timestamp: interactionData.timestamp,
        duration: interactionData.duration,
        progress: interactionData.progress,
        metadata: interactionData.metadata
      }).returning();

      return result[0];
    } catch (error) {
      console.error('Error tracking course interaction:', error);
      throw error;
    }
  }

  async getContentPerformanceMetrics(startDate, endDate) {
    try {
      const interactions = await this.db.select()
        .from(courseInteractions)
        .where(and(
          gte(courseInteractions.timestamp, startDate),
          lte(courseInteractions.timestamp, endDate)
        ));

      const progress = await this.db.select()
        .from(courseProgress)
        .where(and(
          gte(courseProgress.enrollmentDate, startDate),
          lte(courseProgress.enrollmentDate, endDate)
        ));

      const feedback = await this.db.select()
        .from(userFeedback)
        .where(and(
          gte(userFeedback.createdAt, startDate),
          lte(userFeedback.createdAt, endDate)
        ));

      // Group by course
      const courseStats = {};
      
      // Process interactions
      interactions.forEach(interaction => {
        if (!courseStats[interaction.courseId]) {
          courseStats[interaction.courseId] = {
            courseId: interaction.courseId,
            views: 0,
            enrollments: 0,
            completions: 0,
            totalTimeSpent: 0,
            averageRating: 0,
            dropOffPoints: [],
            interactions: []
          };
        }
        
        courseStats[interaction.courseId].interactions.push(interaction);
        
        if (interaction.interactionType === 'video_play') {
          courseStats[interaction.courseId].views++;
        }
        
        if (interaction.duration) {
          courseStats[interaction.courseId].totalTimeSpent += interaction.duration;
        }
      });

      // Process progress
      progress.forEach(prog => {
        if (!courseStats[prog.courseId]) {
          courseStats[prog.courseId] = {
            courseId: prog.courseId,
            views: 0,
            enrollments: 0,
            completions: 0,
            totalTimeSpent: 0,
            averageRating: 0,
            dropOffPoints: [],
            interactions: []
          };
        }
        
        courseStats[prog.courseId].enrollments++;
        
        if (prog.isCompleted) {
          courseStats[prog.courseId].completions++;
        }
        
        if (prog.timeSpent) {
          courseStats[prog.courseId].totalTimeSpent += prog.timeSpent;
        }
      });

      // Process feedback
      feedback.forEach(fb => {
        if (!courseStats[fb.courseId]) {
          courseStats[fb.courseId] = {
            courseId: fb.courseId,
            views: 0,
            enrollments: 0,
            completions: 0,
            totalTimeSpent: 0,
            averageRating: 0,
            dropOffPoints: [],
            interactions: []
          };
        }
        
        const courseFeedback = feedback.filter(f => f.courseId === fb.courseId);
        const avgRating = courseFeedback.reduce((sum, f) => sum + f.rating, 0) / courseFeedback.length;
        courseStats[fb.courseId].averageRating = avgRating;
      });

      // Calculate drop-off points
      Object.values(courseStats).forEach(course => {
        const videoInteractions = course.interactions.filter(i => i.interactionType === 'video_play');
        const progressPoints = videoInteractions.map(i => i.progress || 0);
        
        // Find common drop-off points (where many users stop)
        const progressCounts = {};
        progressPoints.forEach(progress => {
          const roundedProgress = Math.round(progress / 10) * 10; // Round to nearest 10%
          progressCounts[roundedProgress] = (progressCounts[roundedProgress] || 0) + 1;
        });
        
        course.dropOffPoints = Object.entries(progressCounts)
          .sort(([,a], [,b]) => b - a)
          .slice(0, 3)
          .map(([progress, count]) => ({ progress: parseInt(progress), count }));
      });

      // Sort by popularity (views + enrollments)
      const sortedCourses = Object.values(courseStats)
        .sort((a, b) => (b.views + b.enrollments) - (a.views + a.enrollments));

      return {
        totalCourses: sortedCourses.length,
        mostPopularCourses: sortedCourses.slice(0, 10),
        averageCompletionRate: sortedCourses.reduce((sum, c) => 
          sum + (c.enrollments > 0 ? c.completions / c.enrollments : 0), 0) / sortedCourses.length * 100,
        coursesWithDropOffs: sortedCourses.filter(c => c.dropOffPoints.length > 0),
        totalInteractions: interactions.length,
        averageRating: feedback.length > 0 ? 
          feedback.reduce((sum, f) => sum + f.rating, 0) / feedback.length : 0
      };
    } catch (error) {
      console.error('Error getting content performance metrics:', error);
      throw error;
    }
  }

  // Analytics Summary Methods
  async generateDailyAnalytics(date) {
    try {
      const startOfDay = new Date(date);
      startOfDay.setHours(0, 0, 0, 0);
      const endOfDay = new Date(date);
      endOfDay.setHours(23, 59, 59, 999);

      const sessions = await this.db.select()
        .from(userSessions)
        .where(and(
          gte(userSessions.sessionStart, startOfDay),
          lte(userSessions.sessionStart, endOfDay)
        ));

      const progress = await this.db.select()
        .from(courseProgress)
        .where(and(
          gte(courseProgress.enrollmentDate, startOfDay),
          lte(courseProgress.enrollmentDate, endOfDay)
        ));

      const transactions = await this.db.select()
        .from(revenueTransactions)
        .where(and(
          gte(revenueTransactions.createdAt, startOfDay),
          lte(revenueTransactions.createdAt, endOfDay),
          eq(revenueTransactions.status, 'completed')
        ));

      const uniqueUsers = new Set(sessions.map(s => s.userId)).size;
      const totalRevenue = transactions.reduce((sum, t) => sum + parseFloat(t.amount), 0);
      const completions = progress.filter(p => p.isCompleted).length;
      const certificates = progress.filter(p => p.certificateIssued).length;
      const avgSessionDuration = sessions.reduce((sum, s) => sum + (s.duration || 0), 0) / sessions.length;

      await this.db.insert(dailyAnalytics).values({
        id: date.toISOString().split('T')[0],
        date: startOfDay,
        totalUsers: uniqueUsers,
        activeUsers: uniqueUsers,
        newUsers: uniqueUsers, // This should be calculated properly
        totalRevenue: totalRevenue.toString(),
        coursesCompleted: completions,
        certificatesIssued: certificates,
        averageSessionDuration: Math.round(avgSessionDuration),
        bounceRate: 0 // This should be calculated properly
      }).onConflictDoUpdate({
        target: dailyAnalytics.id,
        set: {
          totalUsers: uniqueUsers,
          activeUsers: uniqueUsers,
          totalRevenue: totalRevenue.toString(),
          coursesCompleted: completions,
          certificatesIssued: certificates,
          averageSessionDuration: Math.round(avgSessionDuration)
        }
      });

      return {
        date: startOfDay,
        totalUsers: uniqueUsers,
        activeUsers: uniqueUsers,
        totalRevenue,
        coursesCompleted: completions,
        certificatesIssued: certificates,
        averageSessionDuration: Math.round(avgSessionDuration)
      };
    } catch (error) {
      console.error('Error generating daily analytics:', error);
      throw error;
    }
  }

  async getAnalyticsDashboard(startDate, endDate) {
    try {
      const revenue = await this.getRevenueAnalytics(startDate, endDate);
      const engagement = await this.getUserEngagementMetrics(startDate, endDate);
      const content = await this.getContentPerformanceMetrics(startDate, endDate);

      return {
        revenue,
        engagement,
        content,
        summary: {
          totalRevenue: revenue.totalRevenue,
          totalUsers: engagement.uniqueUsers,
          completionRate: engagement.completionRate,
          averageRating: content.averageRating,
          mostPopularCourse: content.mostPopularCourses[0]?.courseId || null
        }
      };
    } catch (error) {
      console.error('Error getting analytics dashboard:', error);
      throw error;
    }
  }
}

module.exports = AnalyticsService;
